...
km <- pheatmap(mat, kmeans_k = 6, scale= "column", col= colorRampPalette(c("cornflowerblue", "white", "tomato"))(100), 
               breaks= seq(-3, 3, length.out = 101), filename = "pdf_wip/motif_clustering_peSTARRSeq_1.0.pdf", height = 6, width = 30)
...
pdf("pdf_wip/motifs_clusters_additivity_boxplots.pdf", width= 10, height= 12)
...
if(!exists("mot_cmbn"))
{
  ...
  # mot_cmbn <- CJ(mot_L= colnames(km$kmeans$centers)[apply(km$kmeans$centers,2, function(x) any(x>0.5))], mot_R= colnames(km$kmeans$centers)[apply(km$kmeans$centers,2, function(x) any(x>0.5))], unique = T)
