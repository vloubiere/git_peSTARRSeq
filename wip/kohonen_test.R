...
plot.picture <- function(dmat, what = c("col", "truth"), image.dims = c(600, 800), mycols, ...) 
{
what <- match.arg(what)
...
require("grid")
immat <- rgb(dmat[,"red"], dmat[,"green"], dmat[,"blue"], maxColorValue = 255)
...
classes <- c("background", "leaves", "peppers", "peduncles", "stems","sideshoots", "wires", "cuts")
levels(Y) <- classes
...
set.seed(15)
somsegm1 <- supersom(imdata, whatmap = c("rgb", "coords"), user.weights = c(1, 9), grid = mygrid)
...
plot.picture(X32, x = unit(.525, "npc"), just = "left", width = unit(.45, "npc"))

## find class predictions for map units, concentrate only on peppers and leaves
truthpredictions <- classmat2classvec(reconstrIm$unit.predictions$truth)
...
## visualize the rgb part of the map, using unit class labels as background colors
...
plot(somsegm1, "codes", whatmap = "rgb", bgcol = unitcols, palette.name = rgbpalette, main = "")
## add unit numbers and a legend on top
idx <- (0:7)*4 + 1
text(somsegm1$grid$pts[idx,1] - .5, somsegm1$grid$pts[idx,2], labels = idx, pos = 2)
add.cluster.boundaries(somsegm1, truthpredictions)
legend("top", legend = rev(levels(truthpredictions)), bty="n", pch = rep(21, 3), pt.bg = rev(cluscolors), ncol = 3)
## visualize the pixel location part of the map
plot(somsegm1$codes[["coords"]][,1], max(somsegm1$codes[["coords"]][,2]) - somsegm1$codes[["coords"]][,2],
     pch = 21, cex = 3, axes = FALSE, xlab = "", ylab = "", 
     bg = rgb(somsegm1$codes[["rgb"]][,1], somsegm1$codes[["rgb"]][,2], somsegm1$codes[["rgb"]][,3], maxColorValue = 255))
...
for (ii in seq(along = pepperunits)) oneclass[reconstrIm$unit.classif == pepperunits[ii]] <- paste("unit", pepperunits[ii])
oneclass <- factor(oneclass)
nregions <- nlevels(oneclass)
classcols <- c("lightgray", brewer.pal(max(3, nregions), "Set1"))
plot.picture(oneclass, what = "truth", mycols = classcols[1:(nregions + 1)], scales = list(draw = FALSE))
