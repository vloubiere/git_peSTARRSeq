...
# obj <- merge(obj, feat[, ..cols], by.x= "enh_L", by.y= "ID")
# obj <- merge(obj, feat[, ..cols], by.x= "enh_R", by.y= "ID", suffixes= c("_L", "_R"))
# 
# #--------------------------------------------------------------#
# # 2- Test models
# #--------------------------------------------------------------#
# cmd <- "module load r/3.6.2-foss-2018b; Rscript /groups/stark/vloubiere/pipelines/classification_prediction_models.R"
# out_fold <- "/groups/stark/vloubiere/projects/pe_STARRSeq/Rdata/motifs/models/split_enhancers/"
...
dat <- data.table(file= list.files("Rdata/motifs/models/split_enhancers/", ".rds", full.names = T))
