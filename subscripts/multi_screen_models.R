...
# Sample reads smallest sequencing
N_reads <- dat[, sum(umi_counts), .(cdition, CP, spacer)][, min(V1), .(CP, cdition)]
dat <- dat[, .SD[sample(.N, N_reads[.BY, V1, on= c("CP", "cdition")], umi_counts, replace= T), .(L, R)][, .(umi_counts= .N), .(L, R)], .(cdition, CP, spacer)]
dat <- dcast(dat, CP+spacer+L+R~cdition, value.var = "umi_counts", fill = 0)
cols <- grep("input|screen", names(dat), value = T)
dat <- dat[rowSums(dat[, ..cols])>20]
dat[, (cols):= lapply(.SD, as.numeric), .SDcols= cols]
dat[, (cols):= lapply(.SD, function(x) (x+0.5)/sum(x+0.5)*1e6), .(CP, spacer), .SDcols= cols]
# FoldChange
dat[, log2FoldChange:= log2(rowMeans(do.call(cbind, lapply(.SD, function(x) x/input)))), .SDcols= patterns("^screen_rep")]
# Check if individual enhancer is active
control_pairs_log2FC <- dat[grepl("control", L) & grepl("control", R), log2FoldChange, .(CP, spacer)]
dat[, act_wilcox_L:= {
  .c <- log2FoldChange[grepl("control", R)]
  if(length(.c)>5)
    wilcox.test(.c, control_pairs_log2FC[.BY, log2FoldChange, on= c("CP", "spacer")], alternative = "greater")$p.value else
      as.numeric(NA)
}, .(L, CP, spacer)]
dat[, act_wilcox_R:= {
...
dat[, log2FoldChange:= log2FoldChange-median(control_pairs_log2FC[.BY, log2FoldChange, on= c("CP", "spacer")]), .(CP, spacer)]
# Compute expected
dat[, median_L:= .SD[grepl("^control", R), ifelse(.N>5, median(log2FoldChange), as.numeric(NA))], .(L, CP, spacer)]
dat[, median_R:= .SD[grepl("^control", L), ifelse(.N>5, median(log2FoldChange), as.numeric(NA))], .(R, CP, spacer)]
dat[, additive:= log2(2^median_L+2^median_R)]
dat[, multiplicative:= median_L+median_R]
# SAVE
dat <- na.omit(dat[, .(CP, spacer, L, R, log2FoldChange, median_L, median_R, act_wilcox_L, act_wilcox_R, additive, multiplicative)])
saveRDS(dat, 
