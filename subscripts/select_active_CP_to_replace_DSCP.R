...
candidates <- dat[c(46983, 2477, 69043)]
seqs <- getSeq(BSgenome.Dmelanogaster.UCSC.dm3, GRanges(candidates[, .(seqnames, start, end, strand)]))
...
# Neg strand
actCP3_F <- substr(seqs[3], 1, 23)
actCP3_R <- as.character(reverseComplement(DNAString(substr(seqs[3], nchar(seqs[3])-22, nchar(seqs[3])))))

vl_oligo_Tm(actCP1_F)
vl_oligo_Tm(actCP1_R)
vl_oligo_Tm(actCP2_F)
vl_oligo_Tm(actCP2_R)
vl_oligo_Tm(actCP3_F)
vl_oligo_Tm(actCP3_R)
