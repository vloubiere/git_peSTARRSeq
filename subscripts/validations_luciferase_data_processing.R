...
rep <- fread("Rdata/validations_luciferase_plates_replicates_grid.csv", 
             header = T, 
             colClasses = "character", 
             na.strings = "", 
             fill= T)
...
scheme <- scheme[, .SD[, .(row, col, replicate= .GRP), 
                         .(tech_replicate, date, plate)], 
                 Sample_ID]
scheme <- scheme[, !"tech_replicate"]
# ID/sample correspondance
ID <- readRDS("Rdata/validations_luciferase_constructs_metadata.rds")
scheme[ID, c("L", "R"):= .(i.L, i.R), on= "Sample_ID"]
# luciferase data
...
# Mean replicates
dat_all <- dat_all[, .(luc_norm= mean(luc/ren)), .(Sample_ID, L, R, bio_replicate= replicate)]
# Normalize for negative controls
dat_all[, luc_norm:= luc_norm/mean(dat_all[grepl("^control", L) & grepl("^control", R), luc_norm])]
# Compute additive scores
dat_all[, luc_mean_L:= mean(.SD[grepl("^control", R), luc_norm], na.rm= T), .(bio_replicate, L)]
dat_all[, luc_mean_R:= mean(.SD[grepl("^control", L), luc_norm], na.rm= T), .(bio_replicate, R)]
dat_all[, luc_add:= luc_mean_L+luc_mean_R]

saveRDS(dat_all, "Rdata/validations_luciferase_final_table.rds")
