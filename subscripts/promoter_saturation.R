...
# Define weak strong enhancers left and right ----
maxL <- max(dat$indL)
maxR <- max(dat$indR)
pl <- list("Weak 5' enhancers"= dat[between(indL, 1, 1.5), .(ref= L, refInd= indL, enh= R, ind= indR, log2FoldChange)],
           "Strong 5' enhancers"= dat[between(indL, maxL-1, maxL), .(ref= L, refInd= indL, enh= R, ind= indR, log2FoldChange)],
           "Weakest 3' enhancers"= dat[between(indR, 1, 1.5), .(ref= R, refInd= indR, enh= L, ind= indL, log2FoldChange)],
           "Strong 3' enhancers"= dat[between(indR, maxR-1, maxR), .(ref= R, refInd= indR, enh= L, ind= indL, log2FoldChange)])
pl <- rbindlist(pl, idcol = T)
pl[, n:= length(unique(ref)), .id]
pl[, min:= min(refInd), .id]
pl[, max:= max(refInd), .id]
pl[, name:= paste0(.id, "\n(", round(min, 1), "-", round(max, 1), ", n= ", n, ")"), .id]
