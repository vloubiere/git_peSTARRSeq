...
all <- fread("db/final_tables_exp_model/vllib002_short_spacer_peSTARRSeq_max_cutoff_final_oe.txt")
dat <- all[median_L>0.5 & median_R>0.5 & L!=R & !ctl_L & !ctl_R]
...
dat <- merge(dat, 
             ...
             by.y= "ID",
             suffixes= c("__L", "__R"))
dat <- na.omit(dat[, lapply(.SD, function(x) log2(x+1)), R:median_R])
...
#------- With motifs -------#
.flm3 <- paste0(c("log2FoldChange~median_L*median_R", 
                  CJ(grep("__L$", colnames(dat), value = T), grep("__R$", colnames(dat), value = T))[, paste0(V1, "*", V2)]), 
                collapse= "+")
.lm3 <- lm(as.formula(.flm3), 
           dat)
dat[, pred3:= predict(.lm3)]
fwrite(dat, "Rdata/vllib002_modelling_LR_motif_counts.txt")
...
best_fit <- best_fit[FDR<0.00001][order(`t value`)]
...
for(i in seq(nrow(som)))
  best_fit[, name:= gsub(som[i, motif], som[i, BA_cluster], name), name]
...
pdf("pdf/modeling/vllib002_modeling_LR_motif_counts.pdf", 
    width = 15.5, 
    height = 8.5)
