...
sel_motifs <- unique(fisher[padj<0.00001 & estimate>5, variable])

res <- counts[order(as.numeric(rn)), ..sel_motifs]
colnames(res) <- paste0("motif__", colnames(res))
saveRDS(res, "Rdata/informatrive_motifs_300bp_enhancers.rds")
