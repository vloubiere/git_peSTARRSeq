...
dat[, predicted:= log2(2^indL+2^indR-2^basalMean)]
dat[, residuals:= log2FoldChange-predicted]
dat <- dat[grepl("^hk|^shared", L) & grepl("^hk|^shared", R)]
dat$ctlL <- dat$ctlR <- NULL

# Select motifs
ID <- readRDS("db/motif_counts/motifs_IDs.rds")
mot <- readRDS("db/motif_counts/twist012_motif_counts.rds")
setnames(mot,
         names(mot)[-1],
...
pdftools::pdf_combine(c(pdf1, pdf2, pdf3),
                      "pdf/draft/hk_motif_impact_act_vs_res.pdf")