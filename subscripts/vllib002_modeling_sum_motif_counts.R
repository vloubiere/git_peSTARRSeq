...
dat <- rbind(merge(dat, 
             ...
             merge(dat, 
                   ...
                   by.x= "R",
                   by.y= "ID"))
dat <- na.omit(dat[, lapply(.SD, function(x) log2(sum(x)+1)), L:median_R])
...
fwrite(dat, "Rdata/vllib002_modelling_sum_motif_counts.txt")
...
pdf("pdf/modeling/vllib002_modeling_sum_motif_counts.pdf", 
    width = 15.5, 
    height = 8.5)
