...
dir <- "db/sanger_sequencing/vllib017_020/"
...
dat <- dat[grepl("vllib", name) & Date==210921 & project=="pe_STARRSeq"]
...
dat[, refseq:= paste0(ifelse(grepl("vllib017|vllib018", name), DSCP_seq[1], RPS_seq[1]), 
                      constructs["illumina_F", sequence],
                      enh_L, 
                      constructs["CGCov_F", sequence],
                      ifelse(grepl("vllib017|vllib019", name), 
                             constructs["intron2_300bp", sequence], 
                             constructs["intron4_2k", sequence]),
                      constructs["CGCov_R", sequence],
                      enh_R, 
                      vl_revComp(constructs["illumina_R", sequence]),
                      ifelse(grepl("vllib017|vllib018", name), DSCP_seq[3], RPS_seq[3]), 
...
#-------------------------------------------------------#
pdf("pdf/sanger_sequencing/sanger_vllib017_020_210921.pdf")
dat[, {
  vl_sanger_align(refseq, 
                  abfiles = file, 
                  revcomp = as.logical(revComp), 
                  feat_sequences = constructs[c("DSCP", "RPS12", "intron2_300bp", "intron4_2k"), sequence])
  mtext(name, line = 3)
}, .(refseq, name)]
dev.off()
