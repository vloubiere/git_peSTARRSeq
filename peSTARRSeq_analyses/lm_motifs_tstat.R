...
if(!file.exists("Rdata/modeling_peSTARRSeq/motifs_linear_models_results.rds"))
{
  ...
  # SAVE
  saveRDS(res, "Rdata/modeling_peSTARRSeq/motifs_linear_models_results.rds")
}
dat <- readRDS("Rdata/modeling_peSTARRSeq/motifs_linear_models_results.rds")
...
pdf("pdf/peSTARRSeq/motifs_lm_heatmap.pdf", width = 15, height = 14)
...
text(seq(max(.R$coor)), 18, .R[, unique(Dmel2), keyby= coor]$V1, srt= 60, pos= 4, xpd= T, offset= 0)
...
abline(v= 0, lty= 2)
axis(1, c(-10, 0, 10), labels= c(-10, 0, 10), las= 1)
mtext("Left candidate", line = 0.25)
mtext("t value", 1, line = 2)
text(-18, seq(max(.L$coor)), .L[, unique(Dmel1), keyby= coor]$V1, pos= 2, xpd= T, offset= 0)
